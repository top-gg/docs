...
Webhook integrations allow developers to build approved, installable integrations on Top.gg. Integrations use the same [Webhooks](/docs/API/v1/webhooks) system but operate as a bi-directional RPC-like flow during setup, enabling users to connect your service with a single click.
